...
{  							  
	...
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数	
 	num=ch-' ';//得到偏移后的值（ASCII字库是从空格开始取模，所以-' '就是对应字符的字库）
	for(t=0;t<csize;t++)
	{   
...
*@desc: 
******************************************************/
void LCD_ShowInt(u16 x,u16 y,u16 width,u16 height,u8 size, int LCD_Int)
{
	char LCD_Str[10];
	sprintf((char*)LCD_Str,"%5d",LCD_Int);// （u16) 打印到(char)。
	LCD_ShowString(x,y,width,height,size,LCD_Str);
}
...
*@desc: 
******************************************************/
void LCD_ShowFloat(u16 x,u16 y,u16 width,u16 height,u8 size, float LCD_Float)
{
	char LCD_Str[10];
	sprintf((char*)LCD_Str,"%7.2f",LCD_Float);// （float) 打印到(char)。
	LCD_ShowString(x,y,width,height,size,LCD_Str);
}
...
*@Func:	设置字体颜色
******
*@para:	LineColor 线条颜色  BackColor背景颜色
******
*@desc: 传入参数填写颜色英文单词的大写形式（有宏定义）
******************************************************/
void LCD_SetWordColor(u16 LineColor, u16 BackColor)
{
	POINT_COLOR=LineColor;	// 设置字体线条色为红色
	BACK_COLOR=BackColor;  	// 设置字体背景色为白色
}
